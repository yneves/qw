...
module.exports = function (grunt) {
  ...
  var gruntConfig = {
    ...
    watch: {
      ...
      js: {
        files: [].concat(env.config.client.js).concat(env.config.client.jsx).map(function (src) {
    ...
    forever: {
      server: {
        options: {
          index: path.resolve(__dirname, '../bin/qw-server.js'),
          logDir: path.resolve(getTemp(), 'logs')
        }
      }
    },
  ...
  qw.getProductionEnvs().forEach(function (name) {
    grunt.registerTask(name, ['production']);
  });
